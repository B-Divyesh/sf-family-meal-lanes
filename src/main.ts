  ...
  app.innerHTML = `${header()}<main id="main" tabindex="-1">
    ...
    <section class="hero" aria-labelledby="hero-title">
      <div class="hero-copy"><p class="eyebrow">A weekly board for one kitchen</p><h1 id="hero-title">Plan meals by person, not guesswork</h1><p class="lede">For households with different meals, so everyone can see what is theirs and what they share.</p>
        ...
        <ul class="facts"><li>Works offline after setup</li><li>Saved only on this device</li><li>Export a JSON copy</li></ul>
      </div>
...
function openLanes() {
  ...
  dialog.querySelector('[data-action="add-lane"]')?.addEventListener('click', () => { dialog.close('cancel'); plan.lanes.push({ id: crypto.randomUUID(), name: 'New person', color: colors[plan.lanes.length % colors.length] }); openLanes(); });
