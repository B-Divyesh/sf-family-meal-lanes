  ...
  app.innerHTML = `${header()}<main id="main" tabindex="-1">
    ...
    <section class="hero" aria-labelledby="hero-title">
      <div class="hero-copy"><p class="eyebrow">A weekly board for one kitchen</p><h1 id="hero-title">Plan meals by person, not guesswork</h1><p class="lede">For households with different meals, so everyone can see what is theirs and what they share.</p>
        ...
        <ul class="facts"><li>Works offline after setup</li><li>Saved only on this device</li><li>One-time purchase planned</li></ul>
      </div>
    ...
    ${paidSection()}
  </main>${footer()}<div class="sr-only" aria-live="polite">${esc(liveMessage)}</div>${dialogs()}`;
  bindEvents();
}
...
function paidSection() { return `<section class="paid-section" aria-labelledby="paid-title"><p class="eyebrow">One-time purchase</p><h2 id="paid-title">Add as many lanes as your household needs</h2><p>Free plans include Shared plus three people. Pay $12 once for unlimited lanes on this device.</p>${licensed ? '<p class="license-good">Your unlimited-lanes license is active.</p>' : `<div class="paid-actions"><a class="button primary" href="https://api.sociobot.in/api/v1/products/family-meal-lanes/checkout">Buy unlimited lanes for $12</a><form class="license-form" data-form="license"><label for="license-token">Have a license?</label><input id="license-token" name="license" autocomplete="off" placeholder="Paste your license"><button class="button ink">Restore license</button></form></div><p class="small-note">Sociobot is the merchant of record. Refunds are handled at checkout.</p>`}</section>`; }
function boardHtml() {
...
function openLanes() {
  ...
  dialog.innerHTML = `<form method="dialog" class="dialog-sheet" data-form="lanes"><button class="dialog-close" value="cancel" aria-label="Close people form">×</button><p class="eyebrow">People and lanes</p><h2 id="lanes-dialog-title">Name the people who eat here</h2><p class="dialog-intro">Keep Shared for meals several people eat. Add a lane for each person.</p><div class="lane-list">${plan.lanes.map((lane, index) => `<div class="lane-edit"><span class="lane-dot ${lane.color}"></span><label class="sr-only" for="lane-${lane.id}">Lane name</label><input id="lane-${lane.id}" name="lane-name" value="${esc(lane.name)}" maxlength="24"><label class="sr-only" for="color-${lane.id}">Lane color</label><select id="color-${lane.id}" name="lane-color">${colors.map(c => `<option value="${c}" ${c === lane.color ? 'selected' : ''}>${c}</option>`).join('')}</select>${lane.id === 'shared' ? '<span class="fixed">Shared</span>' : `<button type="button" class="remove-lane" data-action="remove-lane" data-id="${lane.id}" aria-label="Remove ${esc(lane.name)}">×</button>`}</div>`).join('')}</div><button type="button" class="button ink" data-action="add-lane">Add person</button><div class="dialog-actions"><span></span><button class="button ink" value="cancel">Cancel</button><button class="button primary" value="default">Save people</button></div></form>`;
  dialog.showModal();
  dialog.addEventListener('close', () => { if (dialog.returnValue === 'default') void saveLanes(dialog); }, { once: true });
  dialog.querySelector('[data-action="add-lane"]')?.addEventListener('click', () => { if (!licensed && plan.lanes.length >= 4) { dialog.close('cancel'); liveMessage = 'The free plan has four lanes. Restore a license or buy unlimited lanes to add another.'; renderLanding(); return; } dialog.close('cancel'); plan.lanes.push({ id: crypto.randomUUID(), name: 'New person', color: colors[plan.lanes.length % colors.length] }); openLanes(); });
  dialog.querySelectorAll<HTMLElement>('[data-action="remove-lane"]').forEach(button => button.addEventListener('click', () => { const id = button.dataset.id!; dialog.close('cancel'); plan.meals = plan.meals.map(m => ({ ...m, laneId: m.laneId === id ? 'shared' : m.laneId, sharedWith: m.sharedWith.filter(x => x !== id) })); plan.lanes = plan.lanes.filter(l => l.id !== id); openLanes(); }));
}
async function saveLanes(dialog: HTMLDialogElement) {
  const names = [...dialog.querySelectorAll<HTMLInputElement>('[name="lane-name"]')]; const laneColors = [...dialog.querySelectorAll<HTMLSelectElement>('[name="lane-color"]')];
  plan.lanes = plan.lanes.map((lane, index) => ({ ...lane, name: names[index]?.value.trim() || `Person ${index + 1}`, color: laneColors[index]?.value as Lane['color'] || lane.color })); await persist('People saved.');
}
async function deleteMeal(meal: Meal) { lastDeleted = meal; plan.meals = plan.meals.filter(m => m.id !== meal.id); await persist('Meal removed. Use Undo to put it back.'); }
...
async function importPlan(file: File) { try { const raw = JSON.parse(await file.text()); const candidate = raw.plan || raw; if (!Array.isArray(candidate.lanes) || !Array.isArray(candidate.meals)) throw new Error('shape'); plan = { lanes: candidate.lanes, meals: candidate.meals, weekOf: typeof candidate.weekOf === 'string' ? candidate.weekOf : mondayISO(), updatedAt: new Date().toISOString() }; await persist('Meal plan imported.'); } catch { liveMessage = 'That file is not a Family Meal Lanes plan. Choose an exported JSON file and try again.'; renderLanding(); } }
...
function bindEvents() {
  ...
  app.querySelector<HTMLFormElement>('[data-form="license"]')?.addEventListener('submit', event => { event.preventDefault(); const form = event.currentTarget as HTMLFormElement; const token = String(new FormData(form).get('license') || '').trim(); if (!token) { liveMessage = 'Paste your license, then restore it.'; renderLanding(); return; } localStorage.setItem(LICENSE_KEY, token); licensed = true; liveMessage = 'License restored. We will check it when you are online.'; renderLanding(); void verifyLicense(token); });
}
async function verifyLicense(token: string) {
  try { const response = await fetch(`https://api.sociobot.in/api/v1/products/family-meal-lanes/verify?license=${encodeURIComponent(token)}`); const result = await response.json() as { valid?: boolean }; localStorage.setItem(LICENSE_CACHE_KEY, JSON.stringify({ valid: result.valid === true, checkedAt: Date.now() })); if (!result.valid) { licensed = false; liveMessage = 'This license is not active. You can buy unlimited lanes or restore another license.'; renderLanding(); } } catch { /* Offline use keeps the latest local license state. */ }
}
function renderLegal(kind: 'privacy' | 'terms') {
  const privacy = kind === 'privacy';
  app.innerHTML = `${header()}<main id="main" class="legal" tabindex="-1"><p class="eyebrow">Family Meal Lanes</p><h1>${privacy ? 'Your meal plan stays on this device' : 'Terms for using this meal board'}</h1>${privacy ? `<p>Family Meal Lanes saves your lanes and meal slips in this browser on this device. It does not create an account or send your plan to us.</p><h2>What is stored</h2><p>Names, meal titles, prep labels, and notes stay in the browser’s local database. You can export a JSON copy or clear browser data to remove it.</p><h2>Demo mode</h2><p>The sample week uses a separate local database. Starting for real does not copy the sample into your plan.</p><h2>Contact</h2><p>Questions about this policy can go to the Param Factory team.</p>` : `<p>This utility is provided for household planning. You are responsible for checking meal choices and food safety for your household.</p><h2>One-time license</h2><p>A $12 license enables unlimited lanes on one device. Sociobot is the merchant of record. Refunds are handled through its checkout and revoke the license.</p><h2>Local data</h2><p>Your browser stores your plan. Export a copy before clearing browser data or moving to another device.</p><h2>No medical advice</h2><p>Prep labels and lanes are planning notes. They do not assess allergens, nutrition, or food safety.</p><h2>Changes</h2><p>We may update this product and these terms as the product changes.</p>`}</main>${footer()}<div class="sr-only" aria-live="polite">${esc(liveMessage)}</div>`;
  bindEvents();
}
...
async function loadRoute() {
  ...
  demo = path === '/demo';
  if (!demo) {
    const query = new URLSearchParams(location.search); const incoming = query.get('license');
    ...
    licensed = Boolean(token) && (!cached || JSON.parse(cached).valid !== false);
    if (token && (!cached || Date.now() - JSON.parse(cached).checkedAt > 86_400_000)) void verifyLicense(token);
  } else licensed = false;
  if (path === '/' || path === '/demo') {
    try { plan = await readPlan(demo) || (demo ? seedPlan() : emptyPlan()); if (!(await readPlan(demo))) await writePlan(); } catch { plan = demo ? seedPlan() : emptyPlan(); liveMessage = 'Storage is unavailable. Changes will not survive this tab.'; }
    renderLanding();
